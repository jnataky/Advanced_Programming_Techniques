...
class Box:
    ...
    def render(self):
        print("* " + str(self._length), "* " + str(self._width))
    ...
    def double(self):
        ...
        return Box(2*self._length, 2*self._width)
    
    def __eq__(self, box):
        
        return (self._length == box._length and self._width == box._width)
    ...
    def combine(self, box):
        ...
        return Box(self._length + box._length, self._width + box._width)
...
class MangoDB:
    ...
    def update_collection(self, collection_name, updates):
        ...
        for i in updates:
            self._db[collection_name][i] = updates[i]
                
        
    def remove_collection(self, specific_collection):
        
        del self._db[specific_collection]
    ...
    def get_collection_size(self, collection_name):
        ...
        return len(self._db[collection_name])
    
    def to_json(self, collection_name):
        
        return json.dumps(self._db[collection_name])
...
def exercise02():
    ...
    r1 = MangoDB()
    r1.add_collection('testscores')
    r1.update_collection('testscores', dict(enumerate(test_scores, start = 1)))
    r1.get_collection_size('testscores')
    r1.list_collection()
    r1.display_all_collections()
    r1.wipe()
    r1.display_all_collections()
...
def exercise03():
    ...
    csv_file = pd.read_csv("https://raw.githubusercontent.com/chainhaus/pythoncourse/master/avocado.csv")
    
    csv_file.to_csv('avocadro.csv')
     
    with open ("avocadro.csv") as reader_obj:
        reader = csv.reader(reader_obj)
